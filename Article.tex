...
\DeclareRobustCommand{\OP}[1]{\begingroup\textcolor{red}{\hl{\textbf{O.P.:} #1}}\endgroup}
\DeclareRobustCommand{\OPP}[1]{\begingroup\textcolor{red}{\hl{\textbf{O.P.:} In previous sentence #1}}\endgroup}
\DeclareRobustCommand{\CR}[1]{\textcolor{red}{#1}}
\DeclareRobustCommand{\SP}[1]{\textcolor{red}{\sout{#1}}}
...
The experimental flow curves were then modeled using Johnson-Cook, Modified Zerilli-Armstrong, Hansel-Spittel, Arrhenius \CR{models}, the recently proposed PTM model, as well as an Artificial Neural Network model with $3$ inputs, $2$ hidden layers of $15$ and $7$ neurons, respectively, and 1 output.
The Mean Absolute Relative Error \CR{($\MARE$)} and the Root Mean Square Error \CR{($\RMSE$)} values were used to quantify the predictive accuracy of the analyzed models.
The results indicated that the Johnson-Cook and Modified Zerilli-Armstrong models presented \CR{a} significant error, \CR{$\MARE$} of $21.20\%$, while the Hansel-Spittel, PTM and Arrhenius models could predict the behavior of this alloy with an error of about $3.56\%$ error.
...
The Hansel-Spittel, PTM and Arrhenius analytical models showed a good interpolation and extrapolation capability\CR{,} with the notable superiority of the Arrhenius model over the other two.
The Artificial Neural Networks model was\CR{,} however, still the most performant among all the models.
\SP{A correlation factor was proposed to extend the applicability of some of the investigated models to deformation conditions not included in the original experimental plan.}\OP{Has to be removed}
\end{abstract}
...
The open die forging process is fundamentally a hot compression process during which work strengthening effects, such as hardening (WH) and precipitation, take place concomitantly with softening phenomena such as recovery and recrystallization under static and/or dynamic \CR{thermomechanical loads} \cite{tian2022deformation, tavakoli2019ferrite}.
It has also been reported that phase transformation can occur during deformation.
The extent and intensity of the above phenomena strongly depend on three macroscopic quantities\CR{,} namely, the strain $\varepsilon$, the strain rate $\mdot\varepsilon$ and the temperature $T$ \cite{ebrahimi2017flow, Ashtiani-2012, He-2013, Changizian-2012}.
...
However, as reported by many authors \cite{Li-2013, Zhang-2015, Zhou-2019}, even after adjusting the constants, the high temperature flow behavior, particularly when dynamic recrystallization takes place, \CR{cannot} be accurately predicted\CR{,} and none of the models is able to accurately predict the flow behavior outside the experimental testing interval that was used to determine the model constants.
Due to its more physics-based formulation, the ZA model and its modified form, MZA\SP{,} \cite{NematNasser-2004, Lennon-2004, Muralli-2017, Cheng-2021, Muralli-2021}, and the Arrhenius-type hyperbolic sine constitutive models are preferred to the JC and HS models both for the prediction of the hot deformation behavior and microstructure analysis of the material \cite{Jonas-1969, Mostafaei-2012, Zhang-2012}.
The Arrhenius formulation has been revised \CR{repeatedly} to achieve a more accurate determination of the activation energy for high temperature deformation \cite{Slooff-2007, Lin-2008-C}.
...
Although progress has been made in improving constitutive models to better predict the flow stress behavior of materials, problem still persist with the \CR{models'} efficiency.
...
In other words, the question is whether interpolation and extrapolation techniques can be used to extend the applicability of a constitutive equation for different \SP{materials and}\OP{Out of this scope concerning different materials} processing windows.

The present work aims to address the preceding question using \SP{a combination of} the recently introduced \SP{PTM model and} ANN \CR{model}.\OP{Objective updated}
...
Then, the interpolation and extrapolation capabilities of each model are evaluated \SP{and a solution proposed, responding to the main question of the paper}.
The results are validated based on experimental work carried out \CR{herein} and on data obtained from the literature.
...
Hot compression tests were performed on a Gleeble-3800 thermomechanical simulator (see Figure \ref{fig:Gleeble3800}), for $5$ temperature levels, namely, $1050\celsius$, $1100\celsius$, $1150\celsius$, $1200\celsius$ and $1250\celsius$\CR{,} with the $6$ strain rates of $0.001~\ps$, $0.01~\ps$, $0.1~\ps$, $1~\ps$, $2.0~\ps$ and $5~\ps$.
\begin{figure}[!ht]
\centering
...
As raw data contains noises, \CR{the} savgol\_filter method from the scipy library is used to remove noise and provide smoother data.
...
Therefore, there are $6$ strain rates and $5$ temperatures, and the database consists of \CR{$21,000$} data points.
The overall behavior of these curves shows that the flow stress $\sigma^y$ increases with increasing strain rate $\mdot\varepsilon$\CR{,} but decreases with increasing temperature $T$.
...
As an example, Figure \ref{fig:Micrography} shows the medium carbon \CR{microstructures} held for $5$~min at $1260\celsius$ before and after the compression phase.
\begin{figure}[!ht]
\centering
...
\SP{The results reported in Figure \ref{fig:RawData},will be used in the next section to identify the parameters of the $6$ proposed flow laws in order to see which mode will matches with experimental.}
...
For the determination of the parameters of the analytical models, the reference values for strain rate and temperature are $\mdot\varepsilon_0=0.001~\ps$ and $T_0=1050\celsius$\CR{,} respectively.
...
For the determination of the constants $B$ and $n$, from the results of the compression test at $T_0$ and $\mdot\varepsilon_0$, these two constants can then \CR{be} determined by considering only the first term $\left(A+B\varepsilon^{p^{n}}\right)$ in Equation (\ref{eq:Johnson-Cook}).
Thus, here, $B=21.816~\MPa$ and $n=0.0746$.
Once the parameters $A$, $B$ and $n$ are known, the determination of $C$, considering \CR{only} all the curves at $T=T_0$ then gives $C=0.3404$.
Finally, the last parameter $m$ is identified from the curves at $\mdot\varepsilon=\mdot\varepsilon_0$ and from knowledge of the \CR{previously identified} parameters and gives $m=0.7057$.
...
\caption{Comparison between the experimental flow stresses and those \CR{predicted} by \CR{the} JC model}
\label{fig:CompExp-JC-6}
\end{figure}
The JC model is unable to describe the evolution of the flow stress of medium carbon for all strain values, and the predicted stresses $\sigma^y$ show a monotonically decreasing trend for several strain \CR{rate} values.
...
\end{equation}
where $\sigma_i^e$ is the experimental value\CR{,} $\sigma_i^p$ is the value predicted using the given model of the flow stress $\sigma^y$ and $N$ is the total number of data points used to compute those coefficients (in our case $N=21~000$).
For the JC model, $\MARE=14.05~\%$ and $\RMSE=12.00~\MPa$.
As reported by Phaniraj \cite{Phaniraj-2003}, the correlation coefficient ($\R$) is generally not a good measure \SP{in our case of study} because it only shows the correlation of the model with respect to the data and not its accuracy, which is a determining factor in the qualification of a model.
Therefore, as a good (high) value of $\R$ (close to $1$) does not necessarily mean a good prediction of the model\CR{,} but simply establishes a good linearity correlation between the experiment and the prediction\CR{,} we avoided its use in our analysis.

%----------------------------------------------------------------------------------
\subsection{The Modified Zerilli-Armstrong model\label{sec:MZA}}
%----------------------------------------------------------------------------------

\CR{The MZA model, which is the modified form of the ZA model, like the JC model presented earlier, is one of the most widely used models implemented in many FEM codes such as the Abaqus software.}\OP{The previous sentence has been completely rewritten!}
...
In the JC model, the parameters are considered separately, while in MZA, they are considered simultaneously\CR{,} and for that formulation, MZA is preferred to the JC model \cite{Hull-2011}.
However, the original form of the ZA model has some limitations due to the fact that it is considered as \CR{a two terms function} (thermal and athermal functions), and to \CR{improve the} formulation, Samantaray \eal \cite{Samantaray-2009} proposed a modified form given by the following equation:
\begin{equation}
\label{eq:MZA-model}
...
To get the parameters of the MZA model, we apply the method proposed by \cite{Samantaray-2009}\CR{,} and the parameters are summarized in \SP{in} Table \ref{tab:MZA}\CR{,} while their predictions are plotted in Figure \ref{fig:CompExp-MZA-6}.
\begin{table}[h!]
\centering{}
\caption{Parameter\SP{s'} constants of Modified Zerilli-Armstrong model}
...
\end{figure}
It can be seen from \CR{this figure} that the MZA model as \CR{well as the} JC model is not \SP{suitable to describe experimental}\CR{able to faithfully reproduce the experimental data} at low strain rates\CR{,} but \CR{is} much better for high strain rates.
The deviation between the predicted values and the experimental values is large because this model has \CR{a} problem correctly describing the softening in its formulation.
For the MZA model, $\MARE=21.20~\%$ and $\RMSE=19.57~\MPa$, showing an overall worse performance than the JC model\CR{,} as reported in \cite{TizeMha-2022}.
...
\end{equation}
where again, $\sigma^y$ is the flow stress, $\varepsilon$ is the strain, $\mdot\varepsilon$ is the strain rate and $T$ is the temperature\CR{,} as proposed earlier.
...
\begin{table}[h!]
\centering
\caption{Parameter\SP{s'} values of the Hansel-Spittel flow law for the medium carbon steel}
\begin{tabular}{ccccc}
	\hline
	        $A$          &         $m_1$          &   $m_2$   &        $m_3$         &         $m_4$         \\
...
This shows that this model is not appropriate for the characterization of this alloy, particularly because of the strong non-linear \SP{the} behavior observed for low strain rate values.
...
To obtain the constitutive equation, all the parameters $A$, $Q$ $\alpha$ and $n$ \CR{must} be determined for a given material.
...
The determination of the order $m$ of the polynomials \SP{defined by}\CR{defining} Equations (\ref{eq:ArA}-\ref{eq:Arn}) \SP{is defined according to}\CR{depends on} the ability of the model to represent the non-linear dependence of the stress \SP{versus the}\CR{on} strain and its generalization.
...
Setting $m=9$ gives the best results\CR{,} and the corresponding parameters are reported in Table \ref{tab:AR}.
\begin{table}[h!]
\centering
\caption{Parameter\SP{s'} values of the Arrhenius flow law for the medium carbon steel}
\begin{tabular}{llll}
	\hline
...
\caption{Comparison between the experimental flow stresses and those predicted \SP{flow stresses} \SP{flow stresses} by the AR model}
...
When establishing its formulation, the main shortcomings of the MZA model were taken into account, in order to render the PTM model flexible for any type of material studied because it removes \CR{the} need for a limited number of parameters as in the MZA model.
...
The determination of the parameters of this model is \SP{calculated}\CR{done} thanks to the LMFIT Python library \cite{Newville-2016} (for more details about this model we refer the reader to our previous work \cite{TizeMha-2022}).
Thus, all the parameters of this model are calculated with $q=5$, $r=5$, $s=1$ and $t=5$.

\CR{Figure }\ref{fig:CompExp-PTM-6} presents a \CR{comparison} of values predicted by the PTM model and experimental values.
...
\begin{table}[h!]
\centering
\caption{Parameter\SP{s'} values of the PTM flow law for the P20 steel}
\begin{tabular}{llll}
	\hline
...
The first is the ability to classify data into different groups\CR{,} and for example\CR{,} to distinguish between images of cats and dogs.
The second corresponds to the universal approximation capacity of these neural networks, which \CR{is} of interest to us \CR{herein}, and thus, to the ability, after learning\CR{,} to predict the flow stress $\sigma^y$ values according to the input data, akin to what is done by the previously identified analytical models.
The main difference is that this approximation is not linked to a fixed mathematical formulation (as in JC, MZA, AR, HS and PTM analytical models), but is only dependent on the data used for training, the number of layers, the number of neurons per layer\CR{,} and the activation functions associated with the neurons of the network.
...
After some tests of different types of network \CR{architectures,} and in accordance with previous works, a network structure with two hidden layers including $15$ neurons for the first hidden layer and $7$ neurons for the second layer give the best compromise between prediction, learning time and model compactness.
...
The Python program used for training the neural network was developed using the specialized Python library\CR{,} Tensorflow \cite{Abadi-2016}.
The Adaptive Moment Estimation (ADAM) optimizer \cite{Kingma-2015} was used for the training phase.
The training data were those from the tests presented in Section \ref{sec:ComTestResults}\CR{,} and were composed of \CR{$21,000$} quadruplets of ($\varepsilon^p$, $\mdot\varepsilon$, $T$, $\sigma^y$) values.
The learning was performed on the basis of \CR{$5,000$} epochs of the experimental data set.
...
\end{figure}
As can be seen in this figure, after $5,000$ \CR{epochs}, we can consider that we have reached a stationary state of the model learning\CR{,} and that it is useless to continue with the learning phase.
...
One can either use the model directly by providing it with new input data, or retrieve the $180$ parameters identified during the training and inject them into a mathematical model based on Equations (\ref{eq:ANN1}) and (\ref{eq:ANN2})\CR{,} which can be implemented in any language (\eg in FORTRAN for use on the Abaqus Explicit FEM code)\CR{,} as proposed in Pantalé \eal \cite{Pantale-2021, Pantale-2023}.
...
\caption{Comparison between the experimental flow stresses and those predicted by \SP{the} the 3-15-7-1-sigmoid ANN model}
...
The identification of the \SP{performed}\CR{previously presented} models is based on a set of experimental data corresponding to 6 strain rates, 5 temperatures and strains ranging from $0$ to $0.7$.
...
On the other hand, since the omitted strain rate ($\mdot\varepsilon=5~\ps$) is outside the range of strain rates for the model identification ($0.001~\ps\text{ to }2~\ps$), we will test the ability of the models to extrapolate results and quantify \CR{deviations} with the experimental values.

%----------------------------------------------------------------------------------
\subsection{Interpolation validation}
%----------------------------------------------------------------------------------

For \SP{the} interpolation validation, the chosen omitted strain rate is $\mdot\varepsilon=1~\ps$ and those used for identification (or training for \CR{the} ANN) are the 5 others, \ie\CR{,} $\mdot\varepsilon=[0.001, 0.01, 0.1, 2, 5]~\ps$.
...
\label{fig:CompInt}
\end{figure}
Table \ref{tab:IntVal} shows the $\MARE$ and $\RMSE$ deviations between the different models and the experimental data calculated either for \CR{the $5$ identified strain rates (lines referred as id. $\mdot\varepsilon$)}, the $6$ strain rates (lines referred as\SP{:} all $\mdot\varepsilon$) or only on the strain rate $\mdot\varepsilon=1~\ps$.
\begin{table}[h!]
\centering{}
...
In Figure \ref{fig:CompInt}\CR{,} it can be seen that the first two models (JC and MZA), presented in this study do not have the ability to reproduce the behavior of the material for the strain rate $\mdot\varepsilon=1~\ps$.
...
This finding is in agreement with the previous findings of Sections \ref{sec:JC} and \ref{sec:MZA}\CR{,} which showed the inability of these models to take into account the softening of the behavior visible at low strain rates and low temperatures.

From a general appearance point of view, the other 4 models, HS, AR, PTM and ANN, give globally similar results\CR{,} with a higher fidelity for the AR and ANN models compared to the other two models.
...
It appears from this \CR{table} that while the two models, AR and ANN, give equivalent (and excellent) results concerning the values of $\MARE$ and $\RMSE$ for the strain rate $\mdot\varepsilon=1~\ps$, the ANN model gives a globally better result across the entire strain \CR{rate} spectrum, with values of $\MARE=0.96\%$ and $\RMSE=1.18~\MPa$ respectively\CR{;} that is to say, values \CR{that are} approximately 2 to 3 times lower for the ANN model than for the AR model.

This shows the superior fidelity of the ANN model \CR{over} to the 5 analytical models presented in this study both in \CR{terms of} the interpolation capability of the model and \CR{of} the overall behavior.
...
\label{fig:CompExt}
\end{figure}
Table \ref{tab:ExtVal} shows the $\MARE$ and $\RMSE$ deviations between the different models and the experimental data calculated either for \CR{the $5$ identified strain rates (lines referred as id. $\mdot\varepsilon$)}, the $6$ strain rates (lines referred as\SP{:} all $\mdot\varepsilon$) or only on the strain rate $\mdot\varepsilon=5~\ps$.
\begin{table}[h!]
\centering{}
...
The HS model performs worse \CR{in} extrapolation than the AR and ANN models, while the PTM model is relegated to the last position in this ranking with values of $\MARE$ and $\RMSE$ greater than $10\%$ for the strain rate $\mdot\varepsilon=5~\ps$.
The two models, AR and ANN, give the best results for the values of $\MARE$ and $\RMSE$ for the strain rate $\mdot\varepsilon=5~\ps$.
\CR{This} time, the AR model performs a little better on the $\mdot\varepsilon=5~\ps$ strain rate as compared to the ANN model, but the latter gives a globally better result across the entire strain \CR{rate} spectrum, with values of $\MARE=1.09\%$ and $\RMSE=2.40~\MPa$ respectively.
...
\OP{Section Proposed technique for better interpolation has been removed in this version.}
%%----------------------------------------------------------------------------------
...
An analysis of the data from the Gleeble trials\SP{,} and \CR{a} comparison of the 6 models proposed in this study against the experiments\SP{,} led to the following conclusions.
...
The dynamic recrystallization (DRX) phenomenon, introduced through the difference between the maximum and permanent \CR{strains}, shows a partially complete microstructure evolution.
...
The ANN model\SP{,} is largely more accurate than the analytical models in predicting the flow stress $\sigma^y$ of \SP{the} medium carbon\CR{,} with a $\MARE=0.62\%$.
...
\SP{These two parts of the study confirm what was demonstrated in the model identifications.}
...
Therefore, we recommend using the ANN model when conducting a study involving this alloy under the experimental conditions examined \CR{herein}.

For future work perspectives, we will develop another ANN model to characterize the microstructure in terms of DRX and the formation of phases such as ferrite, bainite and martensite\CR{,} for example.
...
\SP{In order to complete this paper, we}\CR{We} report here after the computing process and the $180$ coefficients of the Artificial Neural Network ANN-3-15-7-1-sigmoid model used in Section \ref{sec:ANNmodel}.
In order to use this model\CR{,} we describe \CR{here after the details of the procedure to compute the flow stress} $\sigma^y$ from the input variables $\varepsilon^p$, $\mdot\varepsilon$ and $T$\SP{after the procedure to compute the flow stress}.
...
\item Then\CR{,} we compute the output $s$ of the ANN from the input vector $\overrightarrow{x}$ using the following three equations\CR{:}
\begin{equation}
...
\end{array}\right]
\end{equation*}
The \CR{biases} of the first hidden layer $\overrightarrow{b_1}$ is a $15$-component vector:
\begin{equation*}
...
\end{array}\right]
\end{equation*}
The \CR{biases} of the second hidden layer $\overrightarrow{b_2}$ is a 7-component vector:
\begin{equation*}
