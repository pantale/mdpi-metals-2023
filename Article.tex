...
Among these, the Johnson-Cook (JC) \cite{jia2021modified}, Hansel-Spittel (HS) \cite{chadha2018approach}, and Zerilli-Armstrong (ZA) \cite{sim2022modified} models are the best known and most widely available in FEA codes.
...
To circumvent these shortcomings, several modified model forms have been developed \cite{chadha2018approach, liu2021modified, jia2022thermo1, bai2022comparison, zhu2022constitutive}.
...
The equation that describes this model is given as follows \cite{jia2021modified}:
\begin{equation}
...
The Hansel-Spittel model \cite{savaedi2022review} is one of the least known models in terms of integration in FEM codes, although its parameters can more easily be determined than those of the JC or MZA models.
...
The Arrhenius type model \cite{zhou2022hot} is one of the most used models in the framework of material forming, especially when it comes to studying the material microstructure.
...
\end{equation}
where $Z$ is the Zenner-Hollomon parameter \cite{liu2019effect}, $\mdot\varepsilon$ is the strain rate ($\ps$), $Q$ is the apparent activation energy ($\text{J~mol}^{-1}$), $R$ is the universal gas constant ($8.314~\text{J~mol}^{-1} \text{K}^{-1}$), $T$ is the absolute temperature (K), $\sigma^y$ is the flow stress (MPa) for a given strain, strain rate and temperature and $A_1$, $A_2$, $A_3$, $n_1$, $n_2$, $\alpha$ and $\beta=\alpha~n_1$ are dependent on the material.
...
The analytical models selected for this study were the Johnson-Cook (JC) model \cite{jia2021modified}, the Modified Zerilli-Armstrong (MZA) model \cite{Samantaray-2009}, the Hansel-Spittle (HS) model \cite{savaedi2022review}, the Arrhenius (AR) model \cite{zhou2022hot} and the PTM model \cite{TizeMha-2022}.
